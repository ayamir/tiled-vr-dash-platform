...
def start_process(workspace_dir):
    ...
    if len(video_dirs) != 1:
        print(
            f'There are {len(video_dirs)} video directories in path {workspace_dir}')
    else:
        print(
            f'There is {len(video_dirs)} video directory in path {workspace_dir}')
        ...
        if len(tiles) == 0:
            ...
            for tile_name in tiles:
                ...
                output_dir = os.path.join(
                    video_dir, tile_name.replace(".mp4", ""))
